<script lang="ts">
  ...
  async function saveEditComment() {
    ...
    let isOk = await commentManager
      .updateComment(comicId, chapterNumber, comment.id, newContent)
      .then(() => true)
      .catch(() => {
  ...
  async function likeComment() {
    let isOk = await commentManager
      .reactComment(comicId, chapterNumber, comment.id, { type: Reaction.Like })
      .then(() => true)
      .catch(() => {
  ...
  async function dislikeComment() {
    let isOk = await commentManager
      .reactComment(comicId, chapterNumber, comment.id, { type: Reaction.Dislike })
      .then(() => true)
      .catch(() => {
        console.error('Failed to react to comment');
        return false;
  ...
  async function loadReplies() {
    ...
    replies = await commentManager
      .getCommentReplies(comicId, chapterNumber, comment.id)
  ...
  async function postReply() {
    ...
    newReply = await commentManager.getComment(comicId, chapterNumber, newReply.id);
