<script lang="ts">
  ...
  let images: Array<string | ArrayBuffer | null  > = []; // Mảng để lưu trữ các URL của các hình ảnh đã tải lên
  let imageUrl: string;
  let imageData = {
	    File:'',
...
<div class="bg-base-200 flex">
  <div class="flex">
    <img class="self-center" src="../readingmode/testChap.svg" alt="" />
  </div>
  <div class="ml-3 mt-4">
    ...
    <div class="bg-gray-300 w-28 h-7 flex rounded-md mt-2">
      <img class="h-2 self-center ml-2" src="../Status/Complete.svg" alt="" />
      <span class="self-center ml-2 bg-base-200 rounded-full w-4 h-4"></span>
  ...
  <div class="mt-7">
    <div class="flex justify-between mt-7">
      <input
        ...
        class="border-orange-500 h-14 w-72 bg-gray-300 focus:outline-none focus:ring-2 focus:ring-warning custom-input"
      />
      <select
        class="rounded-md h-14 w-72 bg-gray-300 focus:outline-none focus:ring-2 focus:ring-warning"
      >
    ...
    <input
      ...
      class="h-14 w-full bg-gray-300 focus:outline-none focus:ring-2 focus:ring-warning mt-3 custom-input"
    />
  </div>
      ...
      {#each images as imgUrl}
        ...
        >
          <span class=" indicator-item indicator-end">
            <button class="btn btn-xs btn-circle btn-error" on:click={() => removeImage(imgUrl)}>
          ...
          <!-- Hiển thị hình
						ảnh -->
          <img src={'https://i.yomikaze.org'+imgUrl} alt="Uploaded Image" class="max-w-full max-h-full" />
        </div>
      {/each}
    </SortableList>
    <div
