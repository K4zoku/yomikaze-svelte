import { getLatestComics, getPopularComics, getRecentComics, getLatestChapter } from "$utils/comic-utils";
...
export async function load() {
    ...
    const latest = getLatestComics().then(async lastest => {
        let result: ComicWithLastestChapter[] = [];
        for (let comic of lastest) {
